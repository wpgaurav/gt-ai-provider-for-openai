...
class OpenAiModelMetadataDirectory extends AbstractOpenAiCompatibleModelMetadataDirectory
{
    ...
    protected function parseResponseToModelMetadataList(Response $response): array
    {
        ...
        $gptImageOptions = [
            ...
            new SupportedOption(OptionEnum::outputMediaAspectRatio(), ['1:1', '3:2', '2:3', '7:4', '4:7', '16:9', '9:16', '2:1', '1:2']),
            new SupportedOption(OptionEnum::customOptions()),
        ];
        $ttsCapabilities = [
    ...
    protected function modelSortCallback(ModelMetadata $a, ModelMetadata $b): int
    {
        ...
        // Prefer gpt-image-2 (and newer) over gpt-image-1 and dall-e.
        $aImg = (int) (preg_match('/^gpt-image-(\d+)/', $aId, $am) ? $am[1] : 0);
        $bImg = (int) (preg_match('/^gpt-image-(\d+)/', $bId, $bm) ? $bm[1] : 0);
        if ($aImg && $bImg && $aImg !== $bImg) {
            return $aImg > $bImg ? -1 : 1;
        }
        // Dated snapshots of the same family: prefer the most recent snapshot.
        if (preg_match('/^(gpt-image-\d+)-(\d{4}-\d{2}-\d{2})$/', $aId, $aS)
            && preg_match('/^(gpt-image-\d+)-(\d{4}-\d{2}-\d{2})$/', $bId, $bS)
            && $aS[1] === $bS[1]) {
            return strcmp($bS[2], $aS[2]);
        }
